...
export default function PersonCard({ person, showContact = true }: { person: Person; showContact?: boolean }) {
  ...
  const initials = safeName.split(" ").map((n: string) => n[0]).join("").slice(0, 2);
  return (
    <div className="flex items-start gap-4 p-5 rounded-3xl border hover:shadow-lg transition-shadow" style={{ backgroundColor: "#132d1c", borderColor: "#1e4430" }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-bold uppercase tracking-tight font-[family-name:var(--font-barlow)] text-lg" style={{ color: "#f5f5f0" }}>{safeName}</h3>
          {person.portfolioUrl && (
        ...
        <p className="text-sm mt-2 line-clamp-2" style={{ color: "#9db5a5" }}>{safeBio}</p>
